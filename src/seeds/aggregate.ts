...
const aggregateCoreTeams = (parties: { party: string[]; count: number }[]) => {
  ...
  let coreTeams = orderBy(parties, 'count', 'desc');
  ...
  while (coreTeams.length) {
    ...
    for (let i = 0; i < compareTeams.length; i++) {
      if (difference(compareTeams[i].party, team1.party).length === 1) {
    ...
    forEach(coreTeams2, (team2) => {
      ...
      if (!difference(team2.party, team1.party).length) {
        console.log(team2.party, team1.party)
      }

      const flexChar = difference(team2.party, team1.party)[0];
      coreTeam.core_party = intersection(team1.party, team2.party);

      if (!coreTeam.flex) coreTeam.flex = [];
      ...
      if (flexIdx > -1) {
        ...
        if (flexCharIdx > -1) {
          coreTeam.flex[flexIdx][flexCharIdx].count += team2.count;
        } else {
    ...
    if (coreTeam) combinedTeams.push(coreTeam);
    if (prevCoreTeamsLen === coreTeams.length) coreTeams = coreTeams.splice(1);
  }
